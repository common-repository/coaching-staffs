			...
			<?php while ( have_posts() ) : the_post( ); ?>
				...
				<nav id="nav-single">
					...
					<span class="nav-previous">
						...
						if( isset( $back ) && $back != '' ) { 
							echo '<a href="' . $back . '">';?>
							<span class="meta-nav">&larr;</span><?php _e( 'Return', 'mstw-loc-domain' ); ?></a>
						<?php
						}?>
					</span> <!-- .nav-previous -->
				</nav><!-- #nav-single -->
			
			<?php } //End: if( array_key_exists( 'HTTP_REFERER', $_SERVER ) ?>
				...
				// Check to see if there is a postion in the URL
				$position_post = mstw_safe_get( 'position', $_GET, '' );
				...
				if ( $position_post ) {
					...
					// Try to find the (first) position for the coach
					$staffPositionCPT = mstw_cs_find_staff_position( $post );
					
					if ( null === $staffPositionCPT ) {
						$position  = __( 'No Staff Position', 'mstw-loc-domain' );
						$term_list = null;
						
					} else {
						$position  = get_the_title( $staffPositionCPT );
						$term_list = wp_get_post_terms( $staffPositionCPT -> ID, 'staffs' );
						
					}
				}
				
				if ( !empty( $term_list ) ) {
					// received a good staff position (post) in the URL
					$staff_slug = $term_list[0]->slug;
					
				} else {
					// find the first position for the coach (best guess)
					$staff_slug = '';
				}

				$header_class = ( $staff_slug ) ? "coach-header-$staff_slug" : '';
				...
				<div class="coach-header <?php echo( $header_class ) ?>">
